...
install_service() {
  ...
  if [[ "${SRC_DIR}" != "${APP_DIR}" ]]; then
    sudo cp -r "${SRC_DIR}/src" "${SRC_DIR}/config" "${APP_DIR}/"
...
ExecStart=${PYTHON_BIN} -m src.bms_monitor ${APP_DIR}/config/config.yaml
Restart=always
RestartSec=5
...
EOF
  ...
  echo "查看程序日志: tail -f ${APP_DIR}/logs/bms_monitor.log"
}
